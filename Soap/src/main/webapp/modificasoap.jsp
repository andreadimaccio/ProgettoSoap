...
<h1 class="text-center" style="margin-top: 100px;">Modifica Soap-it</h1>
	...
	<form
		...
		<div class="mb-3">
			<label for="categoria" class="form-label">Categoria</label> <select
				...
				<option selected class="<%= soap.getCategorie().getColoreCategoria() %>" value="<%= soap.getCategorie().getIdCategoria()%>"><%= soap.getCategorie().getNomeCategoria() %></option>
				...
				<option class="bianco" value="7">Nessuna Categoria</option>
		...
		<div class="mb-3">
			<label for="data_promemoria">Inserisci una data :</label> <input
				...
				min="" value = "<%= soap.getDataPromemoria()%>">
		</div>


		<div class="mb-3">
			<button type="submit" class="btn btn-primary form-control">Modifica Soap-it</button>
		</div>
